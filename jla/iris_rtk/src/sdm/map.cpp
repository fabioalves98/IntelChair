...
Map::Map(double resolution, size_t cell_size, uint32_t patch_size, bool is3d) :
...
void Map::bounds(Vector3ui& min, Vector3ui& max) const
{
...
bool Map::patchAllocated(const Vector3ui& coordinates) const
{
    return (get(coordinates) != 0);
}

bool Map::patchIsUnique(const Vector3ui& coordinates) const
{
...
void Map::useCompression(bool compression, uint32_t lru_size, const std::string& algorithm)
{
    ...
    if (use_compression_){
        ...
        if (algorithm == "zstd")
            bc_ = new ldc::ZSTDBufferCompressor;
        else if (algorithm == "quicklz")
            bc_ = new ldc::QuickLZBufferCompressor;
        else if (algorithm == "pithy")
            bc_ = new ldc::PithyBufferCompressor;
        else
            bc_ = new ldc::LZ4BufferCompressor;
...
void Map::computeRay(const Vector3ui& from, const Vector3ui& to, VectorVector3ui& sink)
{
    if ( from == to ) return;
...
void Map::computeRay(const Vector3d& from, const Vector3d& to, VectorVector3ui& sink)
{
...
void Map::visit_all_cells(const CellWalker& walker)
{
    for (auto& it : patches){
        auto id = it.first;
        Vector3ui anchor = unhash(id, UNIVERSAL_CONSTANT) * patch_length;
...
void Map::visit_all_cells(const CellWalker& walker) const
{
    for (auto& it : patches){
        auto id = it.first;
        Vector3ui anchor = unhash(id, UNIVERSAL_CONSTANT) * patch_length;
...
uint8_t* Map::get(const Vector3ui& coordinates)
{
    uint64_t idx; // patch index
    Vector3ul rcd = (coordinates / patch_length).template cast<uint64_t>();
...
const uint8_t* Map::get(const Vector3ui& coordinates) const
{
    uint64_t idx; // patch index
...
uint64_t Map::hash(const Vector3ui& coordinates) const
{
    if (is_3d)
...
bool Map::write(const std::string& filename) const
{
#if 0
...
Vector3ui Map::unhash(uint64_t idx, uint64_t stride) const
{
    if (is_3d)
...
bool Map::lru_key_exists(uint64_t idx) const
{
    return lru_items_map_.find(idx) != lru_items_map_.end();
}

void Map::lru_put(uint64_t idx, COWPtr< Container >* container) const
{
...
COWPtr< Container >* Map::lru_get(uint64_t idx) const
{
