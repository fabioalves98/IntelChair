...
Before diving deep in to the instructions on the deployment, it's worth explaining some of the core parts of the system itself for a better understanding of the technologies used and the code devolped along the lifetime of this project.
...
Another one of the implemented nodes is the "cmdvel_parser.cpp". Because the data structure that the microcontroller is expecting in the low level is different from the navigation stack's output commands for the wheelchair's movement, we created a simple node to parse the return values of the navigation stack and match the values that the microcontroller is expecting, in this specific case, joystick x an y values ranging from -100 and 100.
